...
export class JiraService {
  ...
  getAuthUrl(): Observable<{ url: string }> {
    return this.http.get<{ url: string }>(`${this.apiUrl}/jira/auth-url`);
  }

  getStatus(): Observable<JiraStatus> {
    return this.http.get<JiraStatus>(`${this.apiUrl}/jira/status`);
  }
  ...
  getUserStories(projectKey: string): Observable<UserStory[]> {
    return this.http.get<UserStory[]>(`${this.apiUrl}/jira/stories/${projectKey}`);
  }

  disconnect(): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/jira/disconnect`);
  }
}
