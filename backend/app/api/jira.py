...
async def jira_callback(
    ...
    try:
        ...
        print("JIRA CALLBACK ERROR:", str(e))
        return RedirectResponse(
...
async def get_projects(
    ...
    if not cred.jira_api_token:
        raise HTTPException(status_code=400, detail="Missing Jira token")
...
async def get_jira_stories(
    ...
    if not jira_connection.jira_api_token:
        raise HTTPException(status_code=400, detail="Missing Jira token")
    ...
    try:
        stories = await fetch_user_stories(
            jira_connection.jira_api_token,
