...
class ConstraintGuard:
    ...
    def _is_opposite_action_ac(self, ac_lower: str, story_lower: str) -> bool:
        """
        Vérifie si un AC contient une action opposée à celle de la story.
        
        IMPORTANT: Cette méthode est CONSERVATRICE pour éviter les faux positifs.
        Elle utilise des PHRASES COMPLÈTES, pas des mots isolés.
        """
        
        # Structure: (story_keywords, story_exceptions, blocked_phrases, allowed_phrases)
        opposite_rules = [
            # ─────────────────────────────────────────────────────────────
            ...
            (
                ...
                ["connexion et déconnexion", "connexion sécurisée"],
                # Phrases bloquées (actions de login)
                [
                    "se connecter avec",
                ...
                # Phrases autorisées (contexte OK)
                [
                    "après connexion",
            ...
            (
                ...
                # Phrases bloquées (vraies actions d'import)
                [
                ...
                # Phrases autorisées (résultat de l'export)
                [
                    "importés dans",
        ...
        for story_keywords, story_exceptions, blocked_phrases, allowed_phrases in opposite_rules:
            # 1. Check if story is about this action
            story_matches = any(kw in story_lower for kw in story_keywords)
            if not story_matches:
                continue
    
            # 2. Check if story has an exception
            has_exception = any(exc in story_lower for exc in story_exceptions)
            if has_exception:
                continue
    
            # 3. Check allowed phrases FIRST (higher priority)
            for allowed in allowed_phrases:
                if allowed in ac_lower:
                    return False  # Explicitly allowed, not drifted
    
            # 4. Check blocked phrases
